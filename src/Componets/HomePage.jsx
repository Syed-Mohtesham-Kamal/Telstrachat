...
const HomePage = () => {
  ...
  useEffect(()=>{
    if(isConnect && stompClient && auth.reqUser && currentChat){
      const subscription = stompClient.subscribe("/group/"+currentChat.id.toString(),onMessageRevice);
console.log("helllo -------------- ")
      return ()=>{
...
const handleClick = (e) => {
  ...
  }, [chat.createdChat, chat.createdGoup]);
  ...
  return (
    <div className="relative ">
      <div className=" w-full py-14 bg-[#34358f] "></div>
      <div className="flex bg-[#f0f2f5] h-[90vh] absolute left-[2vw] top-[5vh] w-[96vw]">
                    ...
                    <div onClick={() => handleCurrentChat(item)}>
                      ...
                      {item.is_group ? (
                        ...
                        <ChatCard
                          ...
                          // message={
                          //   (item.id ===
                          //     auth.messages[auth.messages.length - 1]?.chat?.id &&
                          //     auth.messages[auth.messages.length - 1]?.content) ||
        ...
        {!currentChat && (
          <div className="w-[70%] flex flex-col items-center justify-center h-full ">
            <div className="max-w-[70%] text-center">
              <h1 className="text-4xl text-gray-600">T chat</h1>
              <p className="my-9">
                send and reveive message without keeping your phone online.              </p>
            </div>
          </div>
        )}
        ...
        {currentChat && (
          <div className="w-[70%] relative bg-blue-200">
            ...
            {/* message section */}
            <div className="px-10 h-[85vh] overflow-y-scroll pb-2">
              <div className="space-y-1 flex flex-col justify-center mt-20 py-2">
                {messages.length>0 && messages?.map((item, i) => (
                  <MessageCard
                    isReqUserMessage={item.user.id!==auth.reqUser.id}
                    content={item.content}
                    username={auth.reqUser?.full_name}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="footer bg-[#f0f2f5] absolute bottom-0 w-full py-2 text-2xl">
              <div className="flex justify-between items-center px-5 relative">
                <BsEmojiSmile className="cursor-pointer" />
                <ImAttachment />

                <input
                  className=" outline-none border-none bg-white pl-4 rounded-md w-[85%]"
                  type="text"
